...
const IconLink: FC<IconLinkProps> = ({ children, label, icon: Icon, className, ...props }) => {
  return (
    <a {...props} className={classNames('flex group relative items-center', className)}>
      <IconContainer className="relative px-2">
        <Icon
          size={32}
          className="text-primary group-hover:scale-90 group-hover:text-secondary transition-color duration-300 ease-in-out"
        />
      </IconContainer>
